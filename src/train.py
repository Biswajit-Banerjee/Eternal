...
def train_epoch(model, train_loader, criterion, optimizer, device, test_loader=None, metrics_tracker=None):
    ...
    with tqdm(train_loader, desc="Training") as pbar:
        for batch_num, batch in enumerate(pbar):
            ...
            if test_loader and (batch_num + 1) % infer_every == 0:
                current_lr = optimizer.param_groups[0]['lr']
                test_metrics = inference(model, test_loader, device)
                metrics_tracker.update(batch_loss, current_lr, test_metrics)
        ...
        with tqdm(val_loader, desc="Validation") as pbar:
            for batch in pbar:
                ...
                for pred, target, length in zip(predictions, tgt, lengths):
                    batch_predictions.append(pred[:length].cpu())
                    batch_targets.append(target[:length].cpu())

                acc  = calculate_metrics(batch_predictions, batch_targets)
